...
int operation(int& worry, string op, string second, string divisible, string yesmonkey, string nomonkey) {
    if (op == "+")
...
void solve(string filename) {
    ...
    for (int i = 0; i < 20; i++) {
        for (int monkey = 0; monkey < index; monkey++) {
            while (items[monkey].size() > 0) {
                inspects[monkey]++;
                int worry = items[monkey].front();
                int toMonkey = operation(worry, instructions[monkey][0], instructions[monkey][1], instructions[monkey][2], instructions[monkey][3], instructions[monkey][4]);
